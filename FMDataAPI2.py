...
class FMDataAPI():
    ...
    def datareadbyID(self):
        ...
        url = self.URL2.format(self.serverAddress, self.databaseName, self.layoutName, 160)
    ...
    def roadPDF(self, url):
        ...
        if response.status_code == 200:
            file = open("/Users/kanyama/test1.pdf","wb")
...
if __name__ == '__main__':
    ...
    if res1 != "":
        ...
        for i in range(n):
            name = json_dict['response']['data'][i]['fieldData']['PDF']
            print(name)
            # url = name[0:name.find('?RCType')]
            url = name
            sf.roadPDF(url)
